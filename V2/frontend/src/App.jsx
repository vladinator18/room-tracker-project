...
export default function App() {
  ...
  const handleEnter = async (e) => {
    ...
    if (cleanEmail) {
      try {
        // Silently check if they have creation privileges
        const isAuthorized = await votingApi.checkWhitelist(cleanEmail);
        setIsWhitelisted(isAuthorized);
        
        // Let EVERYONE in to vote, regardless of whitelist status
        setIsLoggedIn(true);
        setEventView('list');
      } catch (error) {
        setErrorMsg("Error connecting to the database. Please try again.");
      } finally {
        setIsLoading(false);
      }
    } else {
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-sm border p-4 mb-6 flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          {isLoggedIn && activeTab === 'events' && (
            ...
            }} className="flex items-center gap-2 text-slate-500 hover:text-rose-600 text-sm font-medium">
            ...
            {!isLoggedIn ? (
              ...
              <div>
                <div className="flex gap-4 mb-6 border-b pb-4">
                  ...
                  {/* Only show the Create button if they passed the whitelist check */}
                  {isWhitelisted && (

...
export default function App() {
  ...
  // Controls the main top-level tabs
  const [activeTab, setActiveTab] = useState('heatmap'); 
  // Controls the sub-tabs inside the private Event section
  ...
  const handleEnter = async (e) => {
    ...
    try {
      ...
      if (isAuthorized) {
        ...
        setErrorMsg("Access Denied: Email not on the guest list.");
      }
    } catch (error) {
      setErrorMsg("Error checking authorization. Check your database connection.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 md:p-8 font-sans">
      <div className="max-w-6xl mx-auto">
        ...
        {/* View 1: Public Heatmap */}
        {activeTab === 'heatmap' && <Heatmap />}

        {/* View 2: Gated Event Hub */}
        {activeTab === 'events' && (
          <div className="max-w-4xl mx-auto">
            {!isLoggedIn ? (
              <div className="bg-white rounded-xl shadow-sm border p-8 text-center max-w-md mx-auto mt-12">
                ...
                <p className="text-slate-500 mb-6">Enter an authorized email to view polls and cast your vote.</p>
                ...
                <form onSubmit={handleEnter} className="space-y-4">
                  ...
                  <button type="submit" disabled={isLoading} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-lg flex justify-center gap-2 items-center">
                    {isLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : <>Access Polls <ArrowRight className="w-4 h-4" /></>}
                  </button>
                </form>
              </div>
            ) : (
